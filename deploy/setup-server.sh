...
echo "  3. cp .env.production.example .env && nano .env  (secret-уудаа бөглөнө)"
echo "  4. sed -i 's/__DOMAIN__/your-domain.com/g' deploy/nginx/conf.d/app.conf"
echo "  5. bash deploy/init-ssl.sh your-domain.com you@email.com"
echo "  6. docker compose -f docker-compose.prod.yml up -d --build"
echo "  7. docker compose -f docker-compose.prod.yml exec app npx drizzle-kit migrate"
